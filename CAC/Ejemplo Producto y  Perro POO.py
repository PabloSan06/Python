...
class Producto:
    # stock="" aca estoy definiendo una variable de clase,pertenece a la clase siendo comunes a todas las instancias
    # todas las variales de instancia tendran esta de variable de clase ,
    # estas llamandose desde la clase no desde la intancia como variables de instancia
    # serian los variable estaticos de otros lenguajes
    # ejemplo para generar una variable compartida
    # aca sera atributo comun a todas las instancias , ya que lo genere como variable de clase y esta arriba
    # no es definir una variable que la voy a  usar en self.nombre(variable de instancia)
    ...
    def __init__(self, pNombre, pPrecio):
        ...
        # primer metodo que se inicia cuando instancio un producto
        # parametronombreNombre(atributo) y parametroPrecio(atributo)
        # Producto.stock+1=1 si lo definiera como atributo de clase ,cada vez que instancie voy a llamar a mi atributo de clase
    ...
    def __del__(self):
        ...
        # generalmente no lo vamos a utilizar este metodo dado que no eliminara la instancia, dejando de existir el objeto
        print("No sirve mas")

    def __str__(self):
        # metodo de py utilizado para decir la representacion del objeto en un string
        # mismo que toString en java
        # como se retorna  este objeto como un string si lo necesito como tal
...
# mi_producto=none  se dejaria de utilizar la instancia raro
